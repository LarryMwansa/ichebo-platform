...
All Handbook output records carry `record_class: "governance"`. The Handbook is
the only place these types can originate:

| record_type                | Description                                           |
|----------------------------|-------------------------------------------------------|
| `governance_class`         | A branch or category of Kingdom knowledge             |
| `governance_principle`     | A foundational rule or truth                          |
| `governance_concept`       | An idea or theological construct                      |
| `governance_divine_pattern`| A recurring pattern observed in scripture/governance  |
| `governance_narrative`     | A story or account carrying governance meaning        |
| `governance_subject`       | A topic or domain of study                            |
| `governance_entity`        | A named actor, body, or structure                     |
| `governance_framework`     | A structured system of related principles             |
| `governance_protocol`      | A defined sequence of steps for an occasion           |
| `governance_procedure`     | An operational process for recurring tasks            |
| `governance_mandate`       | A directive from the Kingdom Mandate                  |
| `governance_statement`     | A formal declaration or position                      |
| `governance_programme`     | A structured series of activities with a goal         |
| `governance_calendar`      | A time-governed plan of seasons and appointed times   |
...
Record = {
  ...
  record_type:  "journal | prayer | dream | note | bible_note | sermon | governance_class | governance_principle | governance_concept | governance_divine_pattern | governance_narrative | governance_subject | governance_entity | governance_mandate | governance_statement | governance_programme | governance_framework | governance_protocol | governance_procedure | governance_calendar | activity_event | activity_campaign | activity_project | activity_habit | activity_task | learning_course | learning_certification | reference_key | reference_property | custom",
  // Handbook-produced types (record_class must be 'governance', Level 5 only):
  //   governance_class       — a branch or category of Kingdom knowledge
  //   governance_principle   — a foundational rule or truth
  //   governance_concept     — an idea or theological construct
  //   governance_divine_pattern — a recurring pattern observed in scripture/governance
  //   governance_narrative   — a story or account that carries governance meaning
  //   governance_subject     — a topic or domain of study
  //   governance_entity      — a named actor, body, or structure (replaces 'governance_object')
  //   governance_framework   — a structured system of related principles
  //   governance_protocol    — a defined sequence of steps for an occasion
  //   governance_procedure   — an operational process for recurring tasks
  // Note: governance_entity replaces 'governance_object' — avoids JS Object keyword collision
...
The Activity Engine operates on its own objects AND writes records into the Records Engine for persistence and linking. Activities are the execution layer;
